...
- 선행 지표와 후행 지표
  ...
  - 후행 지표: 과거를 설명하는 지표
    ...
    - 스타트업 초기에는 후행 지표가 굉장히 유의미할 수 있음. 행동에 영향을 줄 수 있을 정도의 타당한 선행 지표가 없기 때문이다.
    - ex) 이탈률 -> 이탈률이 높을 경우, 우리 서비스에 문제가 있음.

- 상관 지표와 인과 지표
  - 상관 지표: 어떤 두 지표가 함께 움직인다.
    - 상관 있어 보이는 지표는 매우 많다. 단순하게 수치로만 상관 있다고 판단하지 말 것을 유의함.
...
우리가 하고 싶은 일들이 시장에서 원하는 일인지 그 검증이 필요하다. 이를 위해 린 분석에서는 '린 캔버스'를 제안함.
...
- 각각 개념에 대한 설명은 아래와 같다.
  ...
  - 위의 린 캔버스를 통해 비즈니스 모델 검증을 했다고 해도, 창업자(내)가 정말 저 일을 하고 싶은지가 중요하다.
